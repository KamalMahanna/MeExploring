...
async def fetch_urls(url,proxies,session,a_list_storage,failed_ones):
    
    try:
        async with session.get(url,proxy=proxies, headers={"User-Agent": "Mozilla/5.0"}, timeout=ClientTimeout(total=10)) as response:
...
async def process_batch(urls, a_list_storage,failed_ones):
    async with ClientSession() as session:
        ...
        tasks = [fetch_urls(url,'http://'+np.random.choice(proxies),
...
#----------
all_anime_details = []
asyncio.run(process_batch(['https://myanimelist.net/anime/16498/Shingeki_no_Kyojin',
                           'https://myanimelist.net/anime/16498/Shingeki_no_Kyojin'],all_anime_details,failed_ones))

print(all_anime_details,'\n\n',failed_ones)